describe('Response Helpers - Pure Business Logic Tests', () => {
  // Mock response functions - testing business logic, not implementation
  ...
  const createErrorResponse = (message: string, error?: any, statusCode: number = 500) => ({
    statusCode,
    headers: {
      'Content-Type': 'application/json',
      'Access-Control-Allow-Origin': '*',
  ...
  describe('createErrorResponse', () => {
    ...
    it('should handle non-Error objects', () => {
      ...
      expect(response.body).toContain('"details":{"code":"ECONNREFUSED"}');
    });
  });
