  ...
  describe('Admin Endpoints', () => {
    describe('GET /admin/users', () => {
      it('should list users with pagination', async () => {
        ...
        const result = listUsersTest();

        expect(result.statusCode).toBe(200);
        const responseBody = JSON.parse(result.body);
        expect(responseBody.users).toHaveLength(2);
        expect(responseBody.pagination.totalUsers).toBe(2);
        expect(responseBody.pagination.currentPage).toBe(1);
      });

      it('should handle custom pagination parameters', async () => {
        const listUsersTest = (page: number, limit: number) => {
        ...
        const result = listUsersTest(2, 10);

        expect(result.statusCode).toBe(200);
        const responseBody = JSON.parse(result.body);
        expect(responseBody.users).toHaveLength(10);
        expect(responseBody.pagination.currentPage).toBe(2);
        expect(responseBody.pagination.hasNextPage).toBe(true);
        expect(responseBody.pagination.hasPreviousPage).toBe(true);
      });
    });

    describe('POST /admin/generate-test-data', () => {
      it('should generate test data with default parameters', async () => {
        const generateTestDataTest = () => {
          return {
            statusCode: 200,
            body: JSON.stringify({
        ...
        const result = generateTestDataTest();

        expect(result.statusCode).toBe(200);
        const responseBody = JSON.parse(result.body);
        expect(responseBody.message).toBe('Test data generated successfully');
        expect(responseBody.summary.usersCreated).toBe(5);
        expect(responseBody.summary.postsCreated).toBe(15);
        expect(responseBody.users).toHaveLength(5);
      });

      it('should handle custom parameters', async () => {
        const generateTestDataTest = (userCount: number, postsPerUser: number) => {
          ...
          return {
            ...
            body: JSON.stringify({
              ...
              summary: {
                ...
                totalItems: userCount + totalPosts,
              },
              users: Array.from({ length: userCount }, (_, i) => ({
                userId: `user-${i}`,
                username: `testuser${i}`,
                displayName: `Test User ${i}`,
                postsCount: postsPerUser
              })),
              timestamp: new Date().toISOString(),
            })
          };
        ...
        const result = generateTestDataTest(2, 1);

        expect(result.statusCode).toBe(200);
        const responseBody = JSON.parse(result.body);
        expect(responseBody.summary.usersCreated).toBe(2);
        expect(responseBody.summary.postsCreated).toBe(2);
      });

      it('should validate parameters', async () => {
        const generateTestDataTest = (userCount: string, postsPerUser: string) => {
          const numUsers = parseInt(userCount);
          const numPostsPerUser = parseInt(postsPerUser);

          if (isNaN(numUsers) || isNaN(numPostsPerUser) || numUsers < 1 || numPostsPerUser < 1) {
            return {
              statusCode: 400,
              body: JSON.stringify({ error: 'Validation Error', details: 'Invalid userCount or postsPerUser parameters' })
            };
          }
        ...
        const result = generateTestDataTest('invalid', '3');
    ...
    describe('DELETE /admin/users/{userId}', () => {
      it('should delete user and return count', async () => {
        ...
        const result = deleteUserTest();

        expect(result.statusCode).toBe(200);
        const responseBody = JSON.parse(result.body);
        expect(responseBody.message).toBe('User deleted successfully');
        expect(responseBody.deletedItems).toBe(5);
      ...
      it('should return 404 for non-existent user', async () => {
        const deleteUserTest = () => {
          return {
            statusCode: 404,
            body: JSON.stringify({ error: 'Not Found', details: 'User not found' })
          };
        };
    ...
    describe('DELETE /admin/cleanup', () => {
      it('should cleanup all data', async () => {
        ...
        const result = cleanupTest();

        expect(result.statusCode).toBe(200);
        const responseBody = JSON.parse(result.body);
        expect(responseBody.message).toBe('Cleanup completed successfully');
        expect(responseBody.deletedDynamoItems).toBe(100);
        expect(responseBody.deletedS3Objects).toBe(25);
      });
    });
  });
